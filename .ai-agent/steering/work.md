...
- **フィーチャーブランチ**: タスクごとにデフォルトブランチから作成する。プレフィックス付き kebab-case で命名する
  - `feature/*` - 新機能（例: `feature/add-react-ruleset`）
  - `fix/*` - バグ修正（例: `fix/ts-strict-config`）
  - `refactor/*` - リファクタリング（例: `refactor/modularize-rule-sets`）
...
1. **ブランチ作成**: デフォルトブランチから `git checkout -b {プレフィックス}/{ブランチ名}` で作成
...
- ステップごとに動作確認
- `npm run lint:eslint --workspaces` / `npm test --workspaces` を実行して品質を維持
