...
├── packages/                           # npm パッケージ群（予定）
│   ├── eslint-config-refined/          # メインの共有 ESLint 設定（予定）
│   └── eslint-plugin-promise/          # eslint-plugin-promise v10 書き直し（予定）
├── CLAUDE.md                           # Claude Code 向けプロジェクト説明（予定）
├── README.md                           # プロジェクト README（予定）
└── LICENSE                             # ライセンスファイル（予定）
```
...
メインの共有 ESLint 設定パッケージ。`buildConfig()` ユーティリティを提供し、モジュラーなサブ設定（js, ts, stylistic, imports, node, comments）を組み合わせる。

### packages/eslint-plugin-promise

eslint-plugin-promise の ESLint v10 対応書き直し。カスタムルールを提供する。
